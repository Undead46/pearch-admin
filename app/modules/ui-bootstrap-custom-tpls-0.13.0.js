...
.controller('PaginationController', ['$scope', '$attrs', '$parse', function ($scope, $attrs, $parse) {
  ...
  this.init = function(ngModelCtrl_, config) {
    ...
    if ($attrs.itemsPerPage) {
      $scope.$parent.$watch($parse($attrs.itemsPerPage), function(value) {
        ...
        $scope.itemsPerPage = self.itemsPerPage;
      });
    } else {
  ...
  $scope.noFirstPrev = function() {
    return $scope.page <= $scope.maxSize;
  };

  $scope.noLastNext = function() {
    return $scope.page + ($scope.maxSize - ($scope.maxSize-1)) >= $scope.totalPages;
  };

  $scope.firstCountItems = function() {
    return ($scope.page * $scope.itemsPerPage) - ($scope.itemsPerPage - 1);
  };

  $scope.allCountItems = function() {
    return $scope.page * $scope.itemsPerPage > $scope.totalItems ? $scope.totalItems : $scope.page * $scope.itemsPerPage;
  };
...
.directive('pagination', ['$parse', 'paginationConfig', function($parse, paginationConfig) {
  return {
    ...
    templateUrl: 'js/pagination/pagination.html',
    replace: true,
    link: function(scope, element, attrs, ctrls) {
...
.directive('pager', ['pagerConfig', function(pagerConfig) {
  return {
    ...
    controller: 'PaginationController',
    templateUrl: 'js/pagination/pager.html',
    replace: true,
    link: function(scope, element, attrs, ctrls) {
      var paginationCtrl = ctrls[0], ngModelCtrl = ctrls[1];
...
angular.module("js/pagination/pager.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("js/pagination/pager.html",
    "<ul class=\"pager\">\n" +
...
angular.module("js/pagination/pagination.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("js/pagination/pagination.html",
    "<nav>\n" +
    "  <div>\n" +
    "     <span>{{firstCountItems()}}</span> - <span>{{allCountItems()}}</span> on <span>{{totalItems}}</span>\n" +
    "  </div>\n" +
    "  <ul class=\"pagination\" ng-show=\"totalPage > itemPerPage\">\n" +
    "     <li ng-if=\"directionLinks\" ng-class=\"{disabled: noPrevious()}\"><a href ng-click=\"selectPage(page - 1, $event)\">{{getText('previous')}}</a></li>\n" +
    "     <li ng-if=\"boundaryLinks\" ng-class=\"{disabled: noPrevious()}\" ng-hide=\" noFirstPrev() \"><a href ng-click=\"selectPage(1, $event)\">{{getText('first')}}</a></li>\n" +
    "     <li ng-repeat=\"page in pages track by $index\" ng-class=\"{active: page.active}\"><a href ng-click=\"selectPage(page.number, $event)\">{{page.text}}</a></li>\n" +
    "     <li ng-if=\"boundaryLinks\" ng-hide=\"noLastNext()\" ng-class=\"{disabled: noNext()}\"><a href ng-click=\"selectPage(totalPages, $event)\">{{getText('last')}}</a></li>\n" +
